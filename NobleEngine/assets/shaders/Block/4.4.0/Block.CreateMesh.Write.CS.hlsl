...
cbuffer HeapSlot : register(b0)
{
    ...
    // グループごとの面数のヒープスロット
    uint groupOffsetHeapSlot;
    // 自身のブロック情報が書かれているページ番号一覧のヒープスロット
...
// 共有露出面バッファ(マップ全体で共有)
RWStructuredBuffer<BakedFace> sharedBakedFaces : register(u0);
...
{   
    ...
    // 
    StructuredBuffer<uint> groupOffset = ResourceDescriptorHeap[groupOffsetHeapSlot];
    ...
    // Phase1が記録したこのグループのチャンクローカル開始位置
    uint localBase = groupOffset[groupID.x];
    // このスレッドが担当するブロックの面
    uint faceOut = gFaceOffsetPerThread[threadID];
    ...
    for (uint f = 0; f < 6; f++)
    {
        if (faceMask & (1u << f))
        {
            uint localIndex = localBase + faceOut;
            uint pageLocalIndex = localIndex / pageSize;
            uint slotInPage = localIndex % pageSize;

            // 確保できたページ数を超える分は安全に切り捨てる(プール枯渇時の防御)
            if (pageLocalIndex < myPageCount)
            {
                ...
                sharedBakedFaces[globalSlot] = face;
            }
            faceOut++;
        }
    }
