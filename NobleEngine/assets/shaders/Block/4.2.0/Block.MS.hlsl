...
)
{
    ...
    uint groupIndex = payload.groupIndices[gid.x];
    ...
    // 露出面0なら帰宅
    if (gtid >= faceCount) return;

    // groupIndex * kMaxFacesPerGroup + gtid で各ブロックのユニークなインデックスを取得(ハッシュみたいなもの)
    BakedFace face = bakedFaces[groupIndex * kMaxFacesPerGroup + gtid];
