...
namespace StringConverter
{
    ...
    std::wstring Convert(const std::string& str)
    {
        ...
        auto sizeNeeded = MultiByteToWideChar(CP_UTF8, 0, reinterpret_cast<const char*>(&str[0]), static_cast<int32_t>(str.size()), NULL, 0);
        if (sizeNeeded == 0)
        {
            return std::wstring();
        }
        std::wstring result(sizeNeeded, 0);
        MultiByteToWideChar(CP_UTF8, 0, reinterpret_cast<const char*>(&str[0]), static_cast<int32_t>(str.size()), &result[0], sizeNeeded);
        return result;
    }
