...
bool IsCollision(const Ray& ray, const ModelData& data, const Matrix4x4& worldMatrix)
{
    for (size_t idx = 0; idx + 2 < data.indices.size(); idx += 3)
    {
        auto worldVertex = [&](uint32_t vIndex)
            {
                const Vector4& p = data.vertices[data.indices[idx + vIndex]].position;
                return Transform(Vector3{ p.x, p.y, p.z }, worldMatrix);
            };

        Triangle t;
        t.vertices[0] = worldVertex(0);
        t.vertices[1] = worldVertex(1);
        t.vertices[2] = worldVertex(2);

        bool hit = IsCollision(ray, t);
        if (hit)
        {
			return true;
        }
